...
namespace PipedriveNet.Endpoints
{
    public class OrganizationsEndpoint<TOrganization>
    {
        ...
        private JObject PrepareOrganizationData(string name, string source, string partner, int ownerId)
        {
            return new JObject
            {
                [nameof(name)] = name,
        ...
        {
            var organization = PrepareOrganizationData(name, source, partner, ownerId);

            return _client.Post<TOrganization>("organizations", organization);
        }
    }
}
